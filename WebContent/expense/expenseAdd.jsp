	...
	<base href="<%=basePath%>" />
		...
		<script type="text/javascript">
			...
			function addExpenseItem(id){
				...
				'<option value="通信费用">通信费用</option><option value="办公室耗材">办公室耗材</option><option value="住宿费用">住宿费用</option><option value="房租水电">房租水电</option><option value="其他">其他</option>'+
				'</select></div></td>'+
...
<form action="ExpenseServlet?method=add" method="post">
			<ul class="forminfo">
				<li>
					<label>报销人</label>
				...
				<li>
					<label>真实姓名</label>
					<input name="" type="text" class="dfinput"  value="${sessionScope.emp.realName}" readonly="readonly"/><i></i></li>
				<li>
					<li>
						<!-- <label>报销总额</label>
					...
					<li> -->
									...
									<td>
										<div class="vocation">
											<select class="select1" name="type">
												<option value="通信费用" selected="selected">通信费用</option>
						...
						<li>
							...
							<input name="nextAuditorId" type="text" class="dfinput" value="${sessionScope.emp.mgrid}" readonly="readonly"/>
						</li>
					</li>
					<li>
						<label>总备注信息</label>
			...
			<font style="color: red; font-size: 16px;">${error}</font>
</form>
		</div>
